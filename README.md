...
On that harness green-tea lands in the fast group on Node — with Fastify and NestJS-on-Fastify, and behind raw `http` with no framework at all — and Express and NestJS-on-Express are roughly 3× back. Which member of that fast group tops a given scenario moves between runs by more than the gap between them, so the table below is the answer and a sentence here is not — a ranking written into a README once is a claim that stops being re-measured, and ends up contradicting the file it links to.
