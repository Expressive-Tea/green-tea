...
- **Ceilings that are already on.** `limits` caps body size (`413`), multipart parts, concurrent Node connections (default 1000), and the request / headers / keep-alive timeouts — defaulting to `30s` / `10s` / `5s`, tighter than Node's own two-minute default. Deno and Bun expose no equivalent active-socket cap, so enforce one at the deployment platform or reverse proxy there. Configurable, and safe before you configure anything.
