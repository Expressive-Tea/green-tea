...
**What a teapot exports: steps and routes, and nothing else.** A `@Provider` marked `export: true` fails the boot with an error that says why — a provider's value *is* the object it builds, a pool or a client or a `db`, and the mesh moves data, not objects. Export a `@Step` instead: it runs on the teapot, per request, and its result is what comes back. That is also what makes an absent teapot survivable. A remote step holds nothing between requests, so a teacup whose teapot never answers still boots once the `bootTimeoutMs` grace runs out — it warns, starts without it, and that teapot's steps and routes answer `503` until it arrives. What still fails the boot is a local step that `@needs` a token *nothing* provides, and that is deliberate: it is the check that catches a typo in a token name, which no amount of waiting will fix. The error names the teapots that did not connect, so the two cases read differently.
