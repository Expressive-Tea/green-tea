...
- New public API needs a doc note in `README.md`, **and the page that documents it in [green-tea-docs](https://github.com/Expressive-Tea/green-tea-docs) updated in the same pull request.** The documentation site used to be built by this repository's CI, so a stale page broke the build; it is now a separate repository that imports nothing from here, and nothing fails when the two disagree. Issue #18 happened when the two still lived together, which is the reason this line exists rather than a hope.
