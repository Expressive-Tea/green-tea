...
describe('CORS', () => {
  ...
  it('disallowed origin → no allow-origin; bare OPTIONS (no ACRM) → automatic 204 Allow', async () => {
    @Route('/') class C { @Get('/x') x() { return { ok: 1 }; } }
    @Module({ mountpoint: '/', controllers: [C] }) class CMod {}
    app = createApp({ modules: [CMod], cors: { origins: 'https://a.com' } });
    const server = await app.listen(0);
    ...
    // A bare OPTIONS (no Access-Control-Request-Method) isn't a CORS preflight; the router
    // answers it automatically and advertises the explicit + implicit methods.
    const bare = await fetch(`http://127.0.0.1:${port}/x`, { method: 'OPTIONS' });
    expect(bare.status).toBe(204);
    expect(bare.headers.get('allow')).toBe('GET, HEAD, OPTIONS');
  });
